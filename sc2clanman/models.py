...
class PracticeAttendant(models.Model):
    ...
    player = models.ForeignKey(ClanMember, verbose_name='player')
    event = models.ForeignKey(PracticeEvent, verbose_name='event')
    matchup = models.CharField(_('matchup'), max_length=2, choices=(
        (MU_TVT, _('Terran vs. Terran')),
        (MU_TVZ, _('Terran vs. Zerg')),
        (MU_TVP, _('Terran vs. Protoss')),
        (MU_ZVZ, _('Zerg vs. Zerg')),
        (MU_ZVT, _('Zerg vs. Terran')),
        (MU_ZVP, _('Zerg vs. Protoss')),
        (MU_PVP, _('Protoss vs. Protoss')),
        (MU_PVZ, _('Protoss vs. Zerg')),
        (MU_PVT, _('Protoss vs. Terran')),
    ))
    win = models.BooleanField(_('Did the player win'), default=False)
    notes = models.TextField(_('notes'), blank=True)
    ...
    def __str__(self):
        return '%s %s' % (self.player.get_league_display(), self.player.name)
